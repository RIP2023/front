...
import Ship_Card_Cart from '../../components/Ship_Card_Cart/Ship_Card_Cart.tsx';
...
const  OrderPage: FC = () => {
    ...
    const getData = async () => {
        try {
            const response = await axios(`http://127.0.0.1:8000/orders/${id}/`, {
                method: "GET",
                headers: {
            ...
            for (let i = 0; i < response.data.ships.length;i++ ) {
                if(response.data.ships[i]['photo_data']){}else{
                    response.data.ships[i]['photo_data'] = default_pic
                }}
    ...
    const deleteFromCart = async (ship_id: number) => {
        try {
            await axios(`http://127.0.0.1:8000/links/delete_class_from_order/${ship_id}/`, {
                method: "DELETE",
                headers: {
                    'authorization': session_id
                }
    ...
    const deleteCart = async () => {
        try {
            await axios(`http://127.0.0.1:8000/orders/delete_order/`, {
                method: "PUT",
                headers: {
                    'authorization': session_id
                }
    ...
    const sendCart = async () => {
        try {
            await axios(`http://127.0.0.1:8000/orders/form_order/`, {
                method: "PUT",
                headers: {
                    'authorization': session_id
                }
    ...
    <Container >
        ...
        <Row style={{marginInline:'auto', width:"80%"}}>
        {(data) && data.ships.map((ship: Ship)  => {
                        return (<Ship_Card_Cart
                                    ship_id={ship.ship_id}
                                    name={ship.name}
                                    description={ship.description}
                                    photo_data={ship.photo_data}
                                    imo={ship.imo_of_ship}
